...
import cv2 as cv
# from segmentation.deeplabV3_plus.train import create_model
...
if __name__ == '__main__':
    resdir_path = '../model_result/dataset1_trainset/deeplab_model1'  # 存放模型预测结果
    save_flag = False
    model_weight_path = r'model/model1/deeplab_11_best.pkl'
    # backbone = ['xception', 'mobilenet']
    # net = create_model(2, backbone=backbone[1], pretrained=False)
    net = DeepLabv3_plus(nInputChannels=3, n_classes=2, os=16, pretrained=True, _print=True)
    ...
    with torch.no_grad():
        for test_img, test_label in test_loader:
            ...
            # plt.subplot(1, 3, 1)
            # plt.title('{}'.format(img_name_list[index]))
