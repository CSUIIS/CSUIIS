...
import csv
import cv2 as cv
# from segmentation.deeplabV3_plus.nets import deeplabv3_plus
...
def create_model(num_classes,backbone,pretrained):
    model = deeplabv3_plus.DeepLab(num_classes, backbone=backbone, pretrained=pretrained, downsample_factor=16)
    return model
...
if __name__ == '__main__':
    ...
    # model = create_model(2, backbone=backbone[1], pretrained=False)
    ...
    # optimizer = optim.SGD(model1_t1.parameters(),lr=lr,momentum=0.9,weight_decay=1e-4)
    ...
    for epoch in range(epochs):
        ...
        if (epoch+1) % 20 == 0:
            ...
            torch.save(model.module.state_dict(), os.path.join(save_dir, 'deeplab_{}_{}.pkl'.format(model_num + 1,epoch)))
        ...
        if dice > max_dice and confmat.miou > max_miou:
            ...
            best_epoch = epoch
            print('save')
            torch.save(model.module.state_dict(),os.path.join(save_dir, 'deeplab_{}_best.pkl'.format(model_num + 1)))
    ...
    torch.save(model.module.state_dict(), os.path.join(save_dir, 'deeplab_{}_final.pkl'.format(model_num + 1)))

    plt.figure()
    plt.plot(loss_list)
    plt.savefig(r'../../deeplab_loss_curve.jpg')
    # model1_t1.eval()
