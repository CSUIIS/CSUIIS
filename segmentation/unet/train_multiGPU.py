...
def create_model(num_classes,base_c=32):
    model = UNet(in_channels=3, num_classes=num_classes, base_c=base_c)
    return model
...
if __name__ == '__main__':
    ...
    for epoch in range(epochs):
        ...
        if (epoch+1) % 20 == 0:
            ...
            torch.save(model.module.state_dict(), os.path.join(save_dir, 'unet_{}_{}.pkl'.format(model_num + 1,epoch)))

        print("train epoch[{}] running_loss:{:.3f}".format(epoch + 1, epoch_loss))
        confmat,dice = evaluate(model, val_loader, device, num_classes=2)
        confmat.compute()
        if dice > max_dice or confmat.miou > max_miou:
            max_dice = dice
            max_miou = confmat.miou
            best_epoch = epoch
            print('save')
            torch.save(model.module.state_dict(),os.path.join(save_dir, 'unet_{}_best.pkl'.format(model_num + 1)))
    ...
    torch.save(model.module.state_dict(), os.path.join(save_dir, 'unet_{}_final.pkl'.format(model_num + 1)))

    plt.figure()
    plt.plot(loss_list)
    plt.savefig(r'../../unet_loss_curve.jpg')
    # model1_t1.eval()
